...
```ruby
token = generator.generate(id: user.id)
```
...
```ruby
token = generator.generate(id: user.id, extra_fields: { email_address: user.email_address })
```

### Validating an HMAC

You can verify a token by creating a validator with the same context
and then calling `validate` on it. The initializer is identical to the
generator.
...
```ruby
validator.validate(token, against_id: user.id)
```

Similarly, extra fields can be passed in to be checked.

```ruby
validator.validate(token, against_id: user.id, extra_fields: {
  email_address: user.email_address,
})
```

The `#validate` method will return `true` or `false`. If `false`,
